...
public class Menu {
    ...
    private String promptCoachChoice() {
    ...
    private String promptOrganChoice() {
    ...
    private void runOrganMenu() {
        ...
        while (!choice.equals("5") && !choice.equals("6") && !choice.equals("4") && !choice.equals("3") && !choice.equals("2"));
    }
    ...
    private void fairnessReport() {
        ...
        for (Map.Entry entry : averageHeight.entrySet()) {
            fairnessTeams.put((Team) entry.getKey(), averageHeight.get(entry.getKey()) + averageExperience.get(entry.getKey()));
        }
        ...
        for (Map.Entry team : fairnessTeams.entrySet()) {
            System.out.printf("%s (coached by %s)%n" +
                    ...
                    averageExperience.get(team.getKey()), totalExperience.get(team.getKey()), teams.get(team.getKey()).size(),
        ...
        if (currentTeam != null) {
            ...
            if (playersExist(currentRoster, currentTeam)) {
                ...
                for (Map.Entry entry : heightDistribution.entrySet()) {
                    System.out.printf("%d inches - %d players%n", entry.getKey(), heightDistribution.get(entry.getKey()).size());
                }
                enter();
            }
        }
        ...
        if (currentTeam != null) {
            ...
            if (playersExist(currentRoster, currentTeam)) {
                ...
                do {
                    ...
                    int choice = 0;
                    do {
                        System.out.printf("Please select the number of the player you would like to remove from the %s:%n", currentTeam.getName());
                        do {
                        ...
                        if (choice <= rosterArray.length) {
                            ...
                            System.out.printf("Sorry, there is no player #%d. There are %d players to choose from.%n", choice, rosterArray.length);
                        }
                    } while (choice > rosterArray.length);
                    System.out.printf("Are you sure you'd like to remove %s from the %s?%n", playerChoice.toString(), currentTeam.toString());
                    ...
                    do {
                        ...
                        switch (confirmation) {
                            case "yes":
                                ...
                                System.out.printf("%s removed from %s!%n", playerChoice.toString(), currentTeam.toString());
                    ...
                    if (playersExist(currentRoster, currentTeam)) {
                        System.out.printf("Would you like to remove another player from the %s? (Please enter yes or no)%n", currentTeam.toString());
                        do {
        ...
        if (currentTeam != null) {
            ...
            do {
                if (currentRoster.size() < MAX_PLAYERS) {
                    ...
                    System.out.printf("Please select the number of the player you would like to add to the %s:%n", currentTeam.getName());
                    int choice = 0;
                    do {
                        do {
                            try {
                        ...
                        if (playerExists(players, choice)) {
                            if (!players[choice - 1].isDrafted()) {
                                ...
                                System.out.printf("%s added to %s!%n", players[choice - 1].toString(), currentTeam.getName());
                                enter();
                            } else {
                                System.out.printf("%s is already on a team. Please select a different player or remove %s from his current team.%n",
                    ...
                    System.out.printf("Would you like to add another player to the %s? (yes or no)%n", currentTeam.getName());
                    do {
                        continuing = inputTrimmer();
                        if (continuing.equals("no")) {
                            teams.put(currentTeam, currentRoster);
    ...
    private boolean playersExist(Set<Player> playerList, Team team) {
        if (playerList.isEmpty()) {
            System.out.printf("There are currently no players on %s. Please add some before removing them.%n", team.toString());
            enter();
            return false;
        } else {
            return true;
    ...
    private boolean playerExists(Player[] playerList, int selection) {
        if (selection > playerList.length) {
            System.out.printf("Sorry, there are only %d players to choose from. Please select one of them.%n", playerList.length);
            return false;
        } else {
            return true;
        }
